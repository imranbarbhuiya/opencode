...
- **Review inline**: On pull requests, OpenCode posts findings as a GitHub review with line comments and optional `suggestion` blocks, not only a conversation comment. On later runs it loads its own threads, resolves fixed ones, updates stale comments, and skips duplicates.
- **Secure**: OpenCode runs inside your GitHub's runners.
- **OpenCode 2**: The GitHub Action installs `@opencode-ai/cli` and runs `opencode2 github run`.
...
This will walk you through installing the GitHub app, creating the workflow, and setting up secrets. `opencode github install` still works on the 1.x CLI.
